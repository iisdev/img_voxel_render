...
@njit(fastmath=True)
def ray_casting(screen_array, player_pos, player_angel, player_height, player_pitch, screen_width, screen_height,
                    delta_angle, ray_distance, h_fov, scale_height):
    screen_array[:] = np.array([0, 0, 0])
    y_buffer = np.full(screen_width, screen_height)

    ray_angle = player_angel - h_fov
